...
# Two-locus insecticide-resistance model (ITN + IRS)
PBO_RESISTANCE = {
    'funestus': {
        'heterozygous': 0.67,
        'homozygous': 0.335
...
NET_LOCI = [('PBO-bednet', 'a'), ('IRS', 'b')]

# Initial resistant-allele frequencies per locus
INITIAL_RESISTANCE_FREQ = {
    'funestus': {'a': 0.02, 'b': 0.01},
    'arabiensis': {'a': 0.01, 'b': 0.009},
}
...
# IRS efficacy: organophosphate/neonicotinoid class (non-pyrethroid)
# Decay constant 90 days (Table 1)
IRS_SPEC = dict(
    killing_initial_effect=0.99,
    killing_decay_time_constant=90,
)

# Deployment timing (days from simulation start, which begins Jan 1)
# These offsets recur every 3 years (1095 days)
ITN_START_DAY = 60
IRS_YR2_OFFSET = 500
IRS_YR3_OFFSET = 865

N_BLOCKS = 1  # number of 3-year deployment blocks
BLOCK_DAYS = 3 * 365

# scenarios (IRS deployment timing relative to PBO-bednet)
SCENARIOS = {
    'PBO_only': None,  # no IRS
    'PBO_IRS_yr2': IRS_YR2_OFFSET,  # IRS in year 2 of each block
    'PBO_IRS_yr3': IRS_YR3_OFFSET,  # IRS in year 3 of each block
}
...
def build_camp(scenario, coverage=0.8, serialize=0, sim_duration=9 * 365):
    ...
    Intervention: treatment-seeking, PBO-bednet per block, and IRS in year 2 or 3
    of each block depending on the scenario (None = no IRS).
    """
    ...
    # PBO-bednet: one deployment per 3-year block
    for block in range(N_BLOCKS):
        ...
        add_scheduled_usage_dependent_bednet(
            ...
            insecticide='PBO-bednet',
            discard_config={'Expiration_Period_Distribution': "EXPONENTIAL_DISTRIBUTION",
                            'Expiration_Period_Exponential': 730},
            killing_initial_effect=PBO_SPEC['killing_initial_effect'],
            killing_decay_time_constant=PBO_SPEC['killing_decay_time_constant'],
            blocking_initial_effect=PBO_SPEC['blocking_initial_effect'],
            blocking_decay_time_constant=PBO_SPEC['blocking_decay_time_constant'],
        )

    # IRS: once per 3-year block if the scenario requests it
    irs_first_day = SCENARIOS[scenario]
    if irs_first_day is not None:
        for block in range(N_BLOCKS):
            irs_day = irs_first_day + block * BLOCK_DAYS
            if irs_day < sim_duration:
                add_scheduled_irs_housing_modification(
                    camp,
                    start_day=irs_day,
                    demographic_coverage=coverage,
                    insecticide='IRS',
                    killing_initial_effect=IRS_SPEC['killing_initial_effect'],
                    killing_decay_time_constant=IRS_SPEC['killing_decay_time_constant'],
                )
...
def set_param_fn(config):
    ...
    # per-net, per-locus insecticide resistance (added after both species exist)
    # PBO resistance (locus a)
    ...
    # IRS resistance (locus b)
    for sp in ['funestus', 'arabiensis']:
        add_insecticide_resistance(
            config,
            manifest,
            insecticide_name='IRS',
            species=sp,
            allele_combo=[['b1', 'b1']],
            killing=IRS_RESISTANCE['homozygous']
        )
        ...
        add_insecticide_resistance(
            ...
            species=sp,
            allele_combo=[['b0', 'b1']],
            killing=IRS_RESISTANCE['heterozygous']
        )
